    ...
    if(
        ...
        !empty($data->id)
            ...
            if ($client->validateRequest($data->clientId, $data->sessionToken)) {//validate user request
                // update light
                $switchStatus = $light->updateSwitch($data->id);
                if($switchStatus != -2) {//there is a light device on the database
                    if ($switchStatus != -1) {//no error while updating switch status on database
                        ...
                        if (sendSwitchCommandToDevice($ip, $switchStatus) != -1){ //command successfully executed by Shelly
                            ...
                            // show light data in json format
                            ...
                            if ($device->updateDeviceStatus($data->id, 0)) {
                                $light->updateSwitch($data->id);//update again switch status
                    ...
                    // tell the user no light device found
                    echo json_encode(array("message" => "No light device found."));
                }
            }
            else {
