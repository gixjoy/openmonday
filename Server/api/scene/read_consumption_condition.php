<?php
    ...
    $consCondition = new MeasureCondition($db);
        ...
        !empty($data->sceneId)){
            ...
            if($client->validateRequest($data->clientId, $data->sessionToken)) {
                ...
                while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                    extract($row);
                    $measureStmt = $consCondition->readId($measureConditionId);//get measure record
                    $measureRow = $measureStmt->fetch(PDO::FETCH_ASSOC);
                    extract($measureRow);
                    if($type == "consumption"){//prepare TemperatureCondition to send back to client
                        $consCondition->operator = $operator;
                        $consCondition->value = $value;
                    }
                }
                // set response code - 200 OK
                http_response_code(200);
                echo json_encode($consCondition);
            }
            else {
