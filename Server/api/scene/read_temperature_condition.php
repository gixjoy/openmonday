<?php
    ...
    $tempCondition = new MeasureCondition($db);
        ...
        !empty($data->sceneId)){
            ...
            if($client->validateRequest($data->clientId, $data->sessionToken)) {
                ...
                while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                    extract($row);
                    $measureStmt = $tempCondition->readId($measureConditionId);//get measure record
                    $measureRow = $measureStmt->fetch(PDO::FETCH_ASSOC);
                    extract($measureRow);
                    if($type == "temperature"){//prepare TemperatureCondition to send back to client
                ...
                echo json_encode($tempCondition);
            }
            else {
                ...
                // tell the user
                echo json_encode(array("message" => "Unable to read TemperatureCondition record. Token not valid."));
            }
    }
    else{
