<?php
    ...
    if(
        ...
        !empty($data->time)
            ...
            if ($client->validateRequest($data->clientId, $data->sessionToken)) {//validate user request
                if ($data->time == "now"){//actual humidity
                    getActualValue();
                }
                if ($data->time == "daily"){//daily humidity values
                    getDailyValues();
                    
                }
                if ($data->time == "monthly"){//monthly humidity values
                    getMonthlyValues();
                }
            }
            else {
    ...
    function getActualValue(){
        ...
        $stmt = $measure->readLastRowOfType("humidity");
        if($stmt->rowCount() > 0){
    ...
    function getDailyValues(){
        ...
        $stmt = $measureHourly->readOfType("humidity");
    ...
    function getMonthlyValues(){
        ...
        $measure_arr=array();
        $measure_arr["records"]=array();
        $stmt = $measureDaily->readOfType("humidity");
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            extract($row);
