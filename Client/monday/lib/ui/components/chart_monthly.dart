...
class MonthlyChart extends StatefulWidget {
  ...
  List<ChartMonthlySampleModel> sampleList;
  String type;
  MonthlyChart(this.sampleList, this.type);
...
class _MonthlyChartState extends State<MonthlyChart> {
  ...
  LineChartData mainData() {
    ...
    widget.sampleList.forEach((item){
      FlSpot spot = new FlSpot(double.parse(item.day), double.parse(item.value));
      flValues.add(spot);
    });
    int avg = getAverage(widget.sampleList);
    List<FlSpot> flAVGValues = List();
    widget.sampleList.forEach((item){
      FlSpot spot = new FlSpot(double.parse(item.day), avg.roundToDouble());
    ...
    return LineChartData(
      axisTitleData: FlAxisTitleData(
        ...
        bottomTitle: AxisTitle(
          showTitle: true,
          titleText: R.of(context).chartDays,
          textStyle: TextStyle(
            color: Colors.grey[200],
            fontSize: 12
          )
